...
def main_fixed_mask(MODEL_NAME, dataset, params, net_params, args, imp_num, adj_percent, wei_percent):
    ...
    for epoch in range(start_epoch, params['epochs'] + 1):
        ...
        print(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()) + ' | ' +
              'RP:[{}] (FIX Mask) Epoch:[{}/{}]\t LR:[{:.4f}], LOSS:[{:.4f}], Train:[{:.2f}] Valid:[{:.2f}] Test:[{:.2f}] | Update Test:[{:.2f}] at epoch:[{}]'
    ...
    print("syd :" + '=' * 120)
    print("syd : RP:[{}] (FIX MASK) Final Result Train:[{:.2f}] Valid:[{:.2f}] Test:[{:.2f}] Adj:[{:.2f}%] Wei:[{:.2f}%] | Time:[{:.2f}] min"
...
def main():    
    ...
    for imp_num in range(20):
        ...
        main_fixed_mask(MODEL_NAME, dataset, params, net_params, args, imp_num, adj_percent, wei_percent)
