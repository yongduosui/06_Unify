...
from train.train_COLLAB_edge_classification import train_epoch_sparse as train_epoch_fixed, evaluate_network_sparse as evaluate_network
from train.train_COLLAB_edge_classification import train_epoch_subgradient
import pruning
import copy
import pdb
...
def main_fixed_mask(MODEL_NAME, dataset, params, net_params, args, imp_num, rewind_weight_mask=None, resume_train_ckpt=None):
    ...
    for epoch in range(start_epoch, params['epochs'] + 1):
        ...
        epoch_train_loss, optimizer = train_epoch_fixed(model, optimizer, device, graph, train_edges, params['batch_size'], epoch)
...
"""
    TRAINING GET MASK
"""
def main_get_mask(MODEL_NAME, dataset, params, net_params, args, imp_num, rewind_weight_mask=None, resume_train_ckpt=None):
    ...
    if resume_train_ckpt:
        ...
        ori_model_dict.update(over_lap)
        model.load_state_dict(ori_model_dict)
        print("(GET MASK) Resume at epoch:[{}] len:[{}/{}]!".format(resume_train_ckpt['epoch'], len(over_lap.keys()), len(ori_model_dict.keys())))
        optimizer.load_state_dict(resume_train_ckpt['optimizer_state_dict'])
        adj_spar, wei_spar = pruning.print_sparsity(model)
    else:
    ...
    for epoch in range(start_epoch, params['epochs'] + 1):
        ...
        epoch_train_loss, optimizer = train_epoch_subgradient(model, optimizer, device, graph, train_edges, params['batch_size'], epoch, args, None)
        epoch_train_hits, epoch_val_hits, epoch_test_hits = evaluate_network(
        ...
        print(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()) + ' | ' +
              'IMP:[{}] (GET Mask) Epoch:[{}/{}]\t LR:[{:.4f}], LOSS:[{:.4f}], Train:[{:.2f}] Valid:[{:.2f}] Test:[{:.2f}] | Update Test:[{:.2f}] at epoch:[{}]'
              .format(imp_num, epoch, params['epochs'], optimizer.param_groups[0]['lr'],
                                                        epoch_train_loss, 
                                                        epoch_train_hits[1] * 100,
                                                        epoch_val_hits[1] * 100,
    ...
    print(f"sy: (GET MASK) Train:\nsy: Hits@10: {results['final_train'][0]*100:.4f}% \nsy: Hits@50: {results['final_train'][1]*100:.4f}% \nsy: Hits@100: {results['final_train'][2]*100:.4f}% \n")
    print(f"sy: (GET MASK) Val:\nsy: Hits@10: {results['highest_valid'][0]*100:.4f}% \nsy: Hits@50: {results['highest_valid'][1]*100:.4f}% \nsy: Hits@100: {results['highest_valid'][2]*100:.4f}% \n")
    print(f"sy: (GET MASK) Test:\nsy: Hits@10: {results['final_test'][0]*100:.4f}% \nsy: Hits@50: {results['final_test'][1]*100:.4f}% \nsy: Hits@100: {results['final_test'][2]*100:.4f}% \n")
    
    print('-' * 100)
    print("syd : IMP:[{}] ADJ:[{:.2f}%] WEI:[{:.2f}%] (GET MASK) Final Result Train:[{:.2f}] Valid:[{:.2f}] Test:[{:.2f}] | Time:[{:.2f}] min"
        .format(imp_num, adj_spar, wei_spar, results['final_train'][1] * 100,
                                             results['highest_valid'][1] * 100,
                                             results['final_test'][1] * 100,
                                             (time.time() - t0) / 60))
    print('-' * 100)
...
def main():    
    ...
    parser.add_argument('--model_save_path', type=str, default='imp_setting1_ckpt', help='the directory used to save models')
    parser.add_argument('--save_root', type=str, default='IMP_CKPT', help='experiment name')
    ...
    args.model_save_path = os.path.join(args.save_root, args.model_save_path)
    ...
    for imp_num in range(start_imp, 21):
        ...
        rewind_weight_mask = main_get_mask(MODEL_NAME, dataset, params, net_params, args, imp_num, rewind_weight_mask, resume_train_ckpt)
