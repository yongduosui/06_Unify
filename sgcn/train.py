...
if not pretrain:
    ...
    adj = np.load('admm_{}/adj_{}.npy'.format(FLAGS.dataset, adj_index))
...
if FLAGS.model == 'gcn':
    ...
    print("number of edges * 2 + diag", np.count_nonzero(np.array(adj)))
    num_supports = 1
    model_func = GCN
...
print("Finally Test set results:", "cost=", "{:.5f}".format(test_cost),
      "syd accuracy=", "{:.5f}".format(test_acc), "time=", "{:.5f}".format(test_duration))
