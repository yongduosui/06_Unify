...
prune_list = [5,     9.75,  14.26, 18.55, 22.62, 
              26.49, 30.17, 33.66, 36.98, 40.13,
              43.12, 45.96, 48.67, 51.23, 53.67,
...
model.load("pretrain_{}".format(FLAGS.dataset), sess)
...
U1 = np.zeros_like(Z1)
U2 = np.zeros_like(Z2)
non_zero_idx = np.count_nonzero(np.array(adj))
...
OUTDIR = 'admm_{}'.format(FLAGS.dataset)
if not os.path.exists(OUTDIR): os.makedirs(OUTDIR)

# store adj which accuracy larger than your define
adj_save_dir = OUTDIR + '/adj_' + str(adj_count)
export_dict = {adj_save_dir : cur_adj1}
for filename, var in export_dict.items():
    np.save(filename, var)
print("saving dir {}".format(adj_save_dir))
